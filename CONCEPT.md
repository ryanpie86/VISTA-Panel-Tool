...
- **Yellow/Green wire roles**: settled by the Vista-20P's own technician
  ...
  bench observation briefly suggested the reverse (Yellow showed activity
  on every keypress), but that turned out to be a correlation error —
  Yellow carries the panel's constant alpha-display/zone-status traffic,
  independent of any given keystroke, so something was always happening
  on it regardless. See HARDWARE_ARCHITECTURE.md "Still open" item 1 for
  the full story.
